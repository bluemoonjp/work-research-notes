...
設定の詳細手順・フロー・スコープについては [`kintoneのOAuth認証.md`](kintoneのOAuth認証.md) を参照。

概要:
1. cybozu.com 共通管理 → 「システム管理」→「外部連携」→「高度な連携を設定する」→「OAuth クライアントの追加」
2. クライアント名・リダイレクトエンドポイントを入力して保存
3. 「連携利用ユーザーの設定」で利用を許可するユーザーにチェック
4. 発行されたクライアント ID・クライアントシークレットをアプリ側に設定
