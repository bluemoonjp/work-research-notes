...
- 接続文字列例:
  ...
  ```
- .NET Core / .NET 5 以降（.NET 6/7/8/9/10 含む）は**非対応**（IBM 公式が明示）。モダン .NET 環境では ODBC または Net.IBM.Data.Db2（後述）を使用すること。
...
### 3. Net.IBM.Data.Db2（NuGet / モダン .NET 向け ADO.NET プロバイダー）

**概要**
IBM が提供する Db2 Connect ベースのクロスプラットフォーム ADO.NET プロバイダー。`.NET Data Provider（IBM.Data.DB2.iSeries）` が .NET Core 以降で非対応になるため、モダン .NET 環境での正規代替手段となる。

**主な用途**
- .NET 5 以降（.NET 6/7/8/9/10）アプリからの DB2 for i アクセス
- Linux・macOS 上の .NET アプリからの接続

**C# / .NET サポート**
- NuGet: `Net.IBM.Data.Db2`
- `IBM.Data.Db2.Core` 名前空間を使用
- .NET Standard 2.0 / .NET 5+ 対応。Windows・Linux・macOS で動作
- Db2 Connect ライセンスの要否は構成によって異なる（IBM i への直接接続は要確認）

**参考 URL**
- [NuGet Gallery | Net.IBM.Data.Db2](https://www.nuget.org/packages/Net.IBM.Data.Db2)
- [IBM Data Server Driver for JDBC and SQLJ - IBM Docs](https://www.ibm.com/docs/en/db2/12.1?topic=support-net-ibmdatadb2-nuget-package)

---

### 4. JDBC / JTOpen（IBM Toolbox for Java）
...
**C# / .NET サポート**
- `IBMMQDotnetClient`（NuGet）: MQ ネイティブ API 向け。.NET Standard 2.0 / .NET 6+ 対応
- `IBMXMSDotnetClient`（NuGet）: JMS ライクな XMS API 向け
- MQ 9.3.1 以降は `IBMMQDotnetClient` が標準。XMS API が不要なら `IBMMQDotnetClient` を推奨
...
**概要**
IBM i オブジェクト（QSYS オブジェクト・IFS ファイル・CL コマンド・DB ファイル）を REST API で操作できる仕組み。IBM Rational Developer for i（RDi）や VS Code IBM i 拡張が内部で使用している。
...
**注意**
- IBM i の標準機能ではなく、特定の PTF 適用が必要（V7R5M0: SI83125 等）
- アクセスポートは 2011（HTTP）/ 2012（HTTPS）
- **主に開発ツール（RDi / VS Code 拡張）向け**に設計されており、プロダクション業務への利用は要検証

**参考 URL**
- [Introducing RSE API - IBM Support](https://www.ibm.com/support/pages/introducing-rse-api)
- [IBM i RSE API Administration and Programming Guide (PDF)](https://public.dhe.ibm.com/systems/support/i/iws/systems_i_software_iws_pdf_rseapi_new.pdf)
...
| .NET Data Provider（IBM.Data.DB2.iSeries） | DB2 データアクセス（SQL） | ○（.NET Framework のみ） | .NET Core / .NET 5+ は**非対応**。ACS Windows Package 必須 |
| Net.IBM.Data.Db2（NuGet） | DB2 データアクセス（SQL） | ○（.NET 5+ 対応） | モダン .NET 環境での .NET Data Provider 代替。クロスプラットフォーム |
...
| IBM MQ | 非同期メッセージング | ○（IBMMQDotnetClient / IBMXMSDotnetClient） | エンタープライズ統合向け。別途 MQ 製品が必要 |
