...
- フィールドレベルのアクション（Get field / Set field 等）は TN3270E・TN5250E 専用（ANSI・VT 系では不可）
...
| イベント | 説明 |
|---|---|
| Wait till text appears | 指定テキストが画面に現れるまで待機 |
| Wait till text disappears | 指定テキストが画面から消えるまで待機 |
...
- 日本語環境では CCSID 5026（カタカナ）または CCSID 930（日本語拡張 EBCDIC）を使用することが多い
...
- Bot 内で親 Bot から子 Bot にセッションを共有する場合は「Sharing sessions across bots」機能を使用する
  - 親 Bot で Connect → 子 Bot 呼び出し時にセッション変数を渡す
